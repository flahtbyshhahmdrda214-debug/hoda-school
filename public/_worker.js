...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/health') {
      return new Response(JSON.stringify({ status: 'ok', worker: true, engine: '_worker.js', time: new Date().toISOString() }), {
        headers: {
    ...
    if (url.pathname === '/api/sync/get') {
      ...
      if (!binId) {
        return new Response(JSON.stringify({ error: 'Invalid bin: ' + binName }), {
          status: 400,
